...
# atribuindo todos os valores possivel as variaveis
...
with st.form(key='prediction_form'):
    ...
    valor_total_bem = st.number_input('Valor total do bem', min_value=0.0, 
                                      value=0.0, step=1000.0)
#botão para ação consultar
    submit_button = st.form_submit_button(label='Consultar')

#programando o botão 'consultar'
if submit_button:
    dados_novos = {
        'profissao': [profissao],
...
# criando um json e chamando a API
response = requests.post(url, json=dados_novos)


# Fazendo a previsão via API
